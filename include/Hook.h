...
#define DEFINE_MEMBER_FN_LONG_HOOK(className, functionName, retnType, address_se, address_ae, ...) \
    typedef retnType (className::*_##functionName##_type)(__VA_ARGS__);             \
    static inline uintptr_t* _##functionName##_GetPtrAddr(void) {                   \
        static uintptr_t _address = REL::RelocationID(address_se, address_ae).address();	\
        return &_address;                                                           \
    }                                                                               \
                                                                                    \
...
#define DEFINE_MEMBER_FN_HOOK(functionName, retnType, address_se, address_ae, ...)	\
	DEFINE_MEMBER_FN_LONG_HOOK(_MEMBER_FN_BASE_TYPE, functionName, retnType, address_se, address_ae, __VA_ARGS__)
